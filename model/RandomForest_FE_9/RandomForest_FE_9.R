...
system.time(
  {
    ...
    # ハイパーパラメータを複数指定
    tibble(
      mtry = c(11, 11, 11, 11, 11),
      min_n = c(7, 7, 7, 7, 8),
      trees = c(750, 875, 750, 875, 750),
      max.depth = c(15, 15, 14, 14, 14)
    ) %>%

      # seed average
      tidyr::crossing(seed = c(
        1234,
        sample(1:1000, size = 9, replace = F)
      )) %>%

      # ハイパラ x seed の分だけモデル作成＆予測
      purrr::pmap_dfr(function(mtry, min_n, trees, max.depth, seed) {
