...
add_features_per_category <- function(target_data, train_data) {
  ...
  target_data %>%
    ...
    dplyr::mutate(
      ...
      ratio_position_max_commute = commute / position_max_commute
    ) %>%
    dplyr::select(-position_mean_commute, -position_median_commute, -position_max_commute) %>%

    # area_partner_child_segment
    add_feature_per_category(train_data, area_partner_child_segment, salary, mean) %>%

    # flg_area_partner_child_commute_extra_low
    add_feature_per_category(train_data, flg_area_partner_child_commute_extra_low, salary, median) %>%
...
transform_categories <- function(data) {
  ...
  data %>%
    ...
    dplyr::mutate(
      ...
      education = as.integer(education) - 1L,
      # position_education_partner_segment = as.integer(position_education_partner_segment) - 1L
    ) %>%
